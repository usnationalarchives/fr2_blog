class Wordpress::Option < Wordpress::Base
  ...
  NAMES_TO_SKIP = %w(cron home recently_edited rewrite_rules siteurl upload_path wp_user_roles)
  
  named_scope :dumpable, :conditions => ['NOT (option_name LIKE "\_%" OR option_name LIKE "wpv\_%" OR option_name IN (?))', NAMES_TO_SKIP], :order => "option_name"
